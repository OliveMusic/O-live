...
function makeContext({config,storage,supabase}){
  ...
  const context={
    ...
    OLIVE_RELEASE:{version:'1.3.60',build:190,schemaVersion:14},
...
async function testSchemaUpgradeMessage(){
  ...
  assert.match(elements.earCloudStatus.textContent,/DB-014/);
  assert.equal(dataReadCount,0);
  await elements.cloudSyncNow.listeners.click();
  assert.match(elements.cloudAuthMessage.textContent,/오류 코드 DB-014/);
