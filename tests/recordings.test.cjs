...
assert.match(recorder,/echoCancellation:false,noiseSuppression:false,autoGainControl:true/);
assert.match(recorder,/const CAPTURE_GAIN=2/);
assert.match(recorder,/captureGain\.gain\.value=CAPTURE_GAIN/);
assert.match(recorder,/captureCompressor\.threshold\.value=-8/);
assert.match(recorder,/captureCompressor\.ratio\.value=4/);
assert.match(recorder,/captureCompressor\.connect\(captureDestination\)/);
