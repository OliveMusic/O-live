...
assert.match(index,/id="recordTimeMarker"/);
assert.match(index,/id="recordTimeOlive"/);
assert.match(index,/\.record-time-progress\{[\s\S]*?width:min\(250px,76%\)/);
assert.match(index,/\.record-time-labels\{[^}]*justify-content:space-between; margin-bottom:8px/);
...
assert.match(index,/\.record-time-marker\{[\s\S]*?transform:translate\(-50%,-50%\)/);
assert.match(index,/\.record-time-olive\{[\s\S]*?background:var\(--olive-thumb\)/);
assert.match(index,/\.record-time-progress\.running \.record-time-olive\{ animation:record-time-olive-roll 2s linear infinite; \}/);
assert.match(index,/@keyframes record-time-olive-roll\{ to\{ transform:rotate\(360deg\); \} \}/);
...
assert.match(index,/\.record-player-play\{[\s\S]*?margin-top:18px/);
assert.match(index,/\.record-player-play::before\{[\s\S]*?width:34px; height:28px;/);
assert.match(index,/\.record-player\{[\s\S]*?grid-template-columns:44px minmax\(0,1fr\); align-items:start/);
assert.match(index,/\.record-waveform-wrap\{[\s\S]*?height:64px; padding-top:16px/);
...
assert.match(recorder,/recordTimeMarker\.style\.left=`\$\{\(progress\*100\)\.toFixed\(3\)\}%`/);
assert.match(recorder,/recordTimeFill\.style\.transform=`scaleX\(\$\{progress\.toFixed\(5\)\}\)`/);
assert.doesNotMatch(recorder,/recordTimeOlive\.style\.transform/);
...
assert.match(recorder,/SOUND_TOUCH_PROCESSOR_URL='\.\/vendor\/soundtouch\/soundtouch-processor\.js\?v=174'/);
