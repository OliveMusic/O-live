...
assert.match(index,/\.record-waveform\{[\s\S]*?height:54px/);
...
assert.match(recorder,/isPlaying:\(\)=>draftIsPlaying\(\) \|\| Boolean\(cloudPlayingId\),[\s\S]*?keepWhenHidden:\(\)=>recording \|\| startPending/);
...
assert.match(recorder,/화면이 자동으로 꺼지지 않게 유지합니다 · 튜너 또는 앱 종료 시 중지됩니다/);
...
assert.match(recorder,/cloudFallbackAudio\.loop=Boolean\(region\.enabled/);
