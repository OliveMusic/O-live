...
assert.match(index,/id="recordBackingInput"[^>]*accept="audio\/\*/);
assert.match(index,/id="recordBackingPick"[^>]*>반주 불러오기</);
assert.match(index,/id="recordBackingProgress"[^>]*type="range"/);
assert.match(index,/id="recordBackingAudio"[^>]*playsinline/);
...
assert.match(index,/\.record-backing-play\{[\s\S]*?transform:rotate\(-11deg\)/);
...
assert.match(recorder,/function setupCapture\(ctx\)\{[\s\S]*?setupLevel\(ctx,levelSource\);[\s\S]*?return stream;/);
assert.doesNotMatch(recorder,/captureDestination/);
...
assert.match(recorder,/isPlaying:\(\)=>backingPlaying,[\s\S]*?keepWhenHidden:true/);
...
assert.match(recorder,/잠금화면에서도 녹음을 유지합니다 · 튜너 또는 앱 종료 시 중지됩니다/);
assert.match(recorder,/function loadBackingFile\(file\)/);
assert.match(recorder,/backingAudio\.play\(\)/);
assert.match(recorder,/URL\.createObjectURL\(file\)/);
...
assert.match(audioRuntime,/!transport\.keepWhenHidden/);
