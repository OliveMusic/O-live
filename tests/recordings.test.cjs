...
assert.match(recorder,/const CAPTURE_TARGET_RMS=\.1/);
assert.match(recorder,/const CAPTURE_MAX_GAIN=8/);
assert.match(recorder,/const CAPTURE_NOISE_GATE=\.0007/);
assert.match(recorder,/const CAPTURE_OUTPUT_GAIN=1\.4/);
assert.match(recorder,/function updateCaptureNormalization\(inputRms\)/);
assert.match(recorder,/CAPTURE_TARGET_RMS\/captureEnvelope/);
assert.match(recorder,/inputRms>=CAPTURE_NOISE_GATE/);
assert.match(recorder,/captureInputAnalyser\.connect\(captureGain\)/);
assert.match(recorder,/captureCompressor\.threshold\.value=-14/);
assert.match(recorder,/captureCompressor\.ratio\.value=4/);
assert.match(recorder,/captureCompressor\.connect\(captureOutputGain\)/);
assert.match(recorder,/captureOutputGain\.connect\(captureDestination\)/);
assert.match(recorder,/new MediaRecorder\(processedStream,options\)/);
