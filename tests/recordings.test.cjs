...
assert.match(recorder,/const RECORD_OLIVE_TURN_MS=2000/);
assert.match(recorder,/const rollDegrees=elapsed\/RECORD_OLIVE_TURN_MS\*360/);
assert.match(recorder,/recordTimeOlive\.style\.left=`\$\{\(progress\*100\)\.toFixed\(3\)\}%`/);
assert.match(recorder,/recordTimeFill\.style\.transform=`scaleX\(\$\{progress\.toFixed\(5\)\}\)`/);
assert.match(recorder,/recordTimeOlive\.style\.transform=`translate\(-50%,-50%\) rotate\(/);
...
assert.match(recorder,/SOUND_TOUCH_PROCESSOR_URL='\.\/vendor\/soundtouch\/soundtouch-processor\.js\?v=173'/);
