...
assert.match(index,/\.record-loop-marker::before\{[\s\S]*?font-size:\.6875rem/);
assert.match(index,/\.record-rate-control input\[type="range"\]::\-webkit-slider-thumb\{[\s\S]*?background:var\(--olive-thumb\)/);
...
assert.match(index,/\.record-waveform-wrap\{[\s\S]*?height:68px; padding-top:20px/);
...
assert.match(recorder,/SOUND_TOUCH_PROCESSOR_URL='\.\/vendor\/soundtouch\/soundtouch-processor\.js\?v=170'/);
