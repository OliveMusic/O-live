...
async function preparePage(page,{
  ...
  await page.addInitScript(({
    ...
    if(withCloud==='recordings'){
      ...
      const serverLinks=practiceRows.slice();
      ...
      const client={
        ...
        async rpc(name,args){
          ...
          if(name==='set_practice_link_pinned'){
            const target=serverLinks.find(item=>item.id===args.p_link_id);
            if(target) target.pinned=args.p_pinned;
            return {data:Boolean(target),error:null};
          }
          if(name==='remove_practice_recordings'){
            const ids=new Set(args.p_ids||[]);
            let removed=0;
            for(let i=serverRows.length-1;i>=0;i--){
              if(ids.has(serverRows[i].id)){ serverRows.splice(i,1); removed++; }
            }
            return {data:removed,error:null};
          }
          if(name==='delete_practice_links'){
            const ids=new Set(args.p_ids||[]);
            let removed=0;
            for(let i=serverLinks.length-1;i>=0;i--){
              if(ids.has(serverLinks[i].id)){ serverLinks.splice(i,1); removed++; }
            }
            return {data:removed,error:null};
          }
  ...
  await expect(page.locator('#appVersion')).toHaveText('버전 1.3.66');
}
...
test('분리된 앱이 모바일 화면에서 모든 탭과 서비스 워커를 시작한다',async({page})=>{
  ...
  expect(workerUrl).toContain('service-worker.js?v=196');
});

test('도움말 링크가 나머지 정보 링크 위 줄 가운데에 놓인다',async({page})=>{
  await preparePage(page);
  const help=page.locator('.app-footer .app-help');
  await expect(help).toHaveText('도움말');
  await expect(help).toHaveAttribute('href','guide.html');
  const helpBox=await help.boundingBox();
  const aboutBox=await page.locator('.app-footer a[href="about.html"]').boundingBox();
  const footerBox=await page.locator('.app-footer').boundingBox();
  /* 소개·개인정보처리방침·이용약관보다 위 줄이어야 한다. */
  expect(helpBox.y+helpBox.height).toBeLessThanOrEqual(aboutBox.y+1);
  /* 그 줄에서 가운데여야 한다. */
...
test('버전을 길게 누르면 기기 내 오디오 진단 기록을 복사한다',async({page})=>{
  ...
  expect(payload.release).toEqual({version:'1.3.66',build:196});
...
test('트랙 탭이 기존 올리브 버튼 비율과 계정 연결 흐름을 유지한다',async({page})=>{
  ...
  await expect(page.locator('.record-time-labels')).toHaveCount(0);
...
test('목록 요청이 잠시 실패해도 자동으로 다시 불러온다',async({page})=>{
  await preparePage(page,{
...
test('목록은 접고 펼칠 수 있고 계정 상태에서 같은 관리 화면을 연다',async({page})=>{
  await preparePage(page,{cloudClient:'recordings',microphone:'controls',recordings:[{
    id:'recording-collapse',title:'무제',
    object_path:'recording-browser-user/recording-collapse.m4a',
    duration_ms:4000,byte_size:1000,mime_type:'audio/mp4',waveform:[25,55,35],
    playback_gain:2.5,
    recorded_at:'2026-09-08T00:09:00.000Z',created_at:'2026-09-08T00:09:00.000Z',
  }]});
  await page.locator('.tab-btn[data-tab="trainer"]').click();
  await page.locator('#trainerSeg .seg-btn',{hasText:'트랙'}).click();
...
test('연습 링크 패널은 검색으로 열리고 입력 중에는 요청을 보내지 않는다',async({page})=>{
  await preparePage(page,{cloudClient:'recordings'});
  await page.locator('.tab-btn[data-tab="trainer"]').click();
  await page.locator('#trainerSeg .seg-btn',{hasText:'트랙'}).click();
  await expandRecordList(page);

  /* 하루 100회뿐인 검색 할당량을 입력 중에 태우지 않는지 본다. */
  let searchCalls=0;
  await page.exposeFunction('__oliveCountSearch',()=>{ searchCalls++; });
  await page.evaluate(()=>{
    const cloud=window.OliveCloud;
    const original=cloud.searchYouTube;
    Object.defineProperty(cloud,'searchYouTube',{
      configurable:true,
      value:async query=>{ await window.__oliveCountSearch(); return original.call(cloud,query); },
    });
  });

  const panel=page.locator('#linkPanel');
  await expect(panel).toBeHidden();
  await page.locator('#recordLink').click();
  await expect(panel).toBeVisible();
  /* 기본 진입점은 검색이다. 한 번에 한 폼만 보인다. */
  await expect(page.locator('#linkModeSearch')).toHaveAttribute('aria-selected','true');
  await expect(page.locator('#linkSearchForm')).toBeVisible();
  await expect(page.locator('#linkUrlForm')).toBeHidden();

  await page.locator('#linkSearchInput').fill('아');
  await expect(page.locator('#linkResults')).toBeHidden();
  await page.locator('#linkSearchGo').click();
  await expect(page.locator('#linkMessage')).toHaveText('두 글자 이상 입력해 주세요');
  expect(searchCalls).toBe(0);

  await page.locator('#linkModeUrl').click();
  await expect(page.locator('#linkSearchForm')).toBeHidden();
  await expect(page.locator('#linkUrlForm')).toBeVisible();
  await page.locator('#linkUrlInput').fill('https://example.com/watch?v=dQw4w9WgXcQ');
  await page.locator('#linkUrlGo').click();
  await expect(page.locator('#linkMessage')).toHaveText('YouTube 주소에서 영상을 찾지 못했습니다');
  expect(searchCalls).toBe(0);
});

test('YouTube 재생 상자는 녹음본과 같은 제어 키와 진행 시간 배치를 쓴다',async({page})=>{
  await preparePage(page,{
    cloudClient:'recordings',
    practiceLinks:[{
      id:'youtube-layout-test',provider:'youtube',video_id:'dQw4w9WgXcQ',
      title:'YouTube layout test',duration_ms:200000,last_position_ms:16000,
      loop_a_ms:10000,loop_b_ms:64000,loop_enabled:true,playback_rate:1,
  ...
  await page.locator('#trainerSeg .seg-btn',{hasText:'트랙'}).click();
  await expandRecordList(page);
  const entry=page.locator('.record-entry',{hasText:'YouTube layout test'});
  const row=entry.locator('.record-row');
  await expect(row.getByText('YouTube',{exact:true})).toBeVisible();
  await row.locator('.record-row-open').click();

  const player=entry.locator('.link-player');
  const line=await player.locator('.link-track-line').boundingBox();
  const times=await player.locator('.link-player-times').boundingBox();
  const bounds=await player.boundingBox();
  expect(await player.locator('.link-player-times').textContent()).toBe('0:16/3:20');
  expect(times.y).toBeGreaterThan(line.y+line.height);
  expect(times.y-(line.y+line.height)).toBeLessThanOrEqual(10);
  expect((bounds.x+bounds.width)-(times.x+times.width)).toBeLessThanOrEqual(14);

  const buttonStyle=await player.locator('.record-player-tool').first().evaluate(button=>{
    const style=getComputedStyle(button);
    return {height:button.getBoundingClientRect().height,border:style.borderTopWidth,background:style.backgroundColor};
  });
  expect(buttonStyle.height).toBeCloseTo(28,0);
  expect(buttonStyle.border).toBe('0px');
  expect(buttonStyle.background).not.toBe('rgba(0, 0, 0, 0)');
});

test('목록에서는 녹음본과 YouTube 중 하나만 펼쳐진다',async({page})=>{
  await preparePage(page,{
    cloudClient:'recordings',
    recordings:[{
      id:'recording-1',title:'9월 6일 녹음',object_path:'recording-browser-user/recording-1.m4a',
      duration_ms:69000,byte_size:1000,mime_type:'audio/mp4',waveform:Array.from({length:80},()=>40),
      playback_gain:1,
      recorded_at:'2026-09-06T09:00:00.000Z',created_at:'2026-09-06T09:00:00.000Z',
    }],
    practiceLinks:[{
      id:'link-1',provider:'youtube',video_id:'dQw4w9WgXcQ',
      title:'YouTube 연습',duration_ms:200000,last_position_ms:0,
  ...
  const linkRow=page.locator('.record-entry',{hasText:'YouTube 연습'}).locator('.record-row-open');
  const recordingRow=page.locator('.record-entry',{hasText:'9월 6일 녹음'}).locator('.record-row-open');
  ...
  /* 링크를 열면 녹음본 파형이 닫혀야 한다. */
  await linkRow.click();
  await expect(page.locator('.link-player')).toBeVisible();
...
test('YouTube 배속 슬라이더는 저장값을 보여주고 두 번 누르면 원곡 속도로 돌아간다',async({page})=>{
  await preparePage(page,{
    cloudClient:'recordings',
    practiceLinks:[{
      id:'rate-link',provider:'youtube',video_id:'dQw4w9WgXcQ',
      title:'배속 테스트',duration_ms:200000,last_position_ms:0,
      loop_a_ms:null,loop_b_ms:null,loop_enabled:false,playback_rate:1.5,
      created_at:'2026-09-08T14:00:00.000Z',
    }],
  });
  await page.locator('.tab-btn[data-tab="trainer"]').click();
  await page.locator('#trainerSeg .seg-btn',{hasText:'트랙'}).click();
  await expandRecordList(page);
  await page.locator('.record-entry',{hasText:'배속 테스트'}).locator('.record-row-open').click();
...
test('선택 모드로 녹음본과 YouTube를 한 번에 지운다',async({page})=>{
  await preparePage(page,{
    cloudClient:'recordings',
    recordings:[
      {id:'keep-recording',title:'남길 녹음',object_path:'recording-browser-user/keep-recording.m4a',
      ...
      {id:'drop-recording',title:'지울 녹음',object_path:'recording-browser-user/drop-recording.m4a',
        duration_ms:30000,byte_size:1000,mime_type:'audio/mp4',waveform:[],playback_gain:1,
        recorded_at:'2026-09-08T09:00:00.000Z',created_at:'2026-09-08T09:00:00.000Z'},
    ],
    practiceLinks:[{
      id:'drop-link',provider:'youtube',video_id:'dQw4w9WgXcQ',
      title:'지울 링크',duration_ms:200000,last_position_ms:0,
      loop_a_ms:null,loop_b_ms:null,loop_enabled:false,playback_rate:1,
  ...
  await page.locator('#recordSelect').click();
  await expect(page.locator('#recordSelectBar')).toBeVisible();
  await expect(page.locator('#recordSelectDelete')).toBeDisabled();

  /* 녹음본과 링크를 섞어서 고를 수 있다. */
  const pick=title=>page.locator('.record-entry',{hasText:title}).locator('.record-select-box input');
  await pick('지울 녹음').check();
  await pick('지울 링크').check();
  await expect(page.locator('#recordSelectCount')).toHaveText('2개 선택');
  await expect(page.locator('#recordSelectDelete')).toBeEnabled();

  await page.locator('#recordSelectDelete').click();
  await expect(page.locator('#recordList .record-entry')).toHaveCount(1);
  await expect(page.locator('#recordList')).toContainText('남길 녹음');
...
test('고정한 항목이 최신 항목보다 위에 온다',async({page})=>{
  await preparePage(page,{
    cloudClient:'recordings',
    recordings:[{
      id:'new-recording',title:'최신 녹음',object_path:'recording-browser-user/new-recording.m4a',
      duration_ms:30000,byte_size:1000,mime_type:'audio/mp4',waveform:[],playback_gain:1,
      recorded_at:'2026-09-09T09:00:00.000Z',created_at:'2026-09-09T09:00:00.000Z',
    }],
    practiceLinks:[{
      id:'old-link',provider:'youtube',video_id:'dQw4w9WgXcQ',
      title:'오래된 링크',duration_ms:200000,last_position_ms:0,
...
test('A/B 없이 반복을 켜면 처음부터 끝까지 반복한다',async({page})=>{
  await preparePage(page,{
    cloudClient:'recordings',
    practiceLinks:[{
      id:'link-whole',provider:'youtube',video_id:'dQw4w9WgXcQ',
      title:'전체 반복 테스트',duration_ms:200000,last_position_ms:0,
      loop_a_ms:null,loop_b_ms:null,loop_enabled:false,playback_rate:1,
      created_at:'2026-09-08T14:00:00.000Z',
    }],
  });
  ...
  await page.locator('#trainerSeg .seg-btn',{hasText:'트랙'}).click();
  await expandRecordList(page);
  await page.locator('.record-entry',{hasText:'전체 반복 테스트'}).locator('.record-row-open').click();

  const repeat=page.locator('.link-player [data-role="repeat"]');
  /* A/B가 없어도 눌러서 켤 수 있어야 한다. */
  await expect(repeat).toBeEnabled();
  await expect(repeat).toHaveAttribute('aria-label','전체 반복 켜기');
  await expect(repeat).toHaveAttribute('aria-pressed','false');
  await repeat.click();
  await expect(repeat).toHaveAttribute('aria-pressed','true');
  await expect(repeat).toHaveAttribute('aria-label','반복 끄기');
...
test('녹음 줄을 열면 올리브 재생 버튼과 탐색 가능한 파형이 나타난다',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>({
    ...
    module:'./vendor/soundtouch/soundtouch-processor.js?v=196',
  ...
  await page.mouse.up();
  await expect(page.locator('.record-player-elapsed')).toHaveText('0:00');
...
test('녹음 배속 처리기가 실제 브라우저 AudioWorklet에 등록된다',async({page})=>{
  ...
  const result=await page.evaluate(async()=>{
    ...
    try{
      await ctx.audioWorklet.addModule('./vendor/soundtouch/soundtouch-processor.js?v=196');
...
test('목록 재생은 녹음 시작 시 정지되고 잠금화면에서도 녹음 상태를 유지한다',async({page})=>{
  ...
  await backingPlay.click();
  await expect(backingPlay).not.toHaveClass(/playing/);
  await expect(page.locator('#recordMessage')).toHaveText('녹음을 정지한 뒤 목록을 재생해 주세요');
...
test('데스크톱에서는 오디오 파일을 목록 카드에 놓아 업로드한다',async({page})=>{
...
test('보통보다 약한 녹음 입력도 음량 막대에 충분히 보인다',async({page})=>{
  ...
  const recordButton=await page.locator('#recordToggle').boundingBox();
  ...
  expect(recordButton.y+recordButton.height).toBeLessThan(progressBar.y);
  expect(progressBar.y-(recordButton.y+recordButton.height)).toBeGreaterThanOrEqual(12);
...
test('멈춰 둔 목록 재생 뒤 청음 트레이너가 일반 출력으로 재생된다',async({page})=>{
  await preparePage(page,{
