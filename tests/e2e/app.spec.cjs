...
async function preparePage(page,{
  ...
  await expect(page.locator('#appVersion')).toHaveText('버전 1.3.62');
}
...
test('분리된 앱이 모바일 화면에서 모든 탭과 서비스 워커를 시작한다',async({page})=>{
  ...
  expect(workerUrl).toContain('service-worker.js?v=192');
});
...
test('버전을 길게 누르면 기기 내 오디오 진단 기록을 복사한다',async({page})=>{
  ...
  expect(payload.release).toEqual({version:'1.3.62',build:192});
...
test('녹음 줄을 열면 올리브 재생 버튼과 탐색 가능한 파형이 나타난다',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>({
    ...
    module:'./vendor/soundtouch/soundtouch-processor.js?v=192',
...
test('녹음 배속 처리기가 실제 브라우저 AudioWorklet에 등록된다',async({page})=>{
  ...
  const result=await page.evaluate(async()=>{
    ...
    try{
      await ctx.audioWorklet.addModule('./vendor/soundtouch/soundtouch-processor.js?v=192');
