...
assert.match(index,/service-worker\.js\?v=100/);
assert.match(index,/cloud-sync\.js\?v=97/);
assert.match(worker,/const VERSION = 'v100'/);
...
assert.match(index,/nav\.tabbar button\.sounding::after[\s\S]*background:radial-gradient\(circle,/);
