...
const schemaContract=fs.readFileSync('supabase/011_recording_mime_types.sql','utf8');
...
assert.match(schemaContract,/allowed_mime_types=excluded\.allowed_mime_types/);
assert.match(schemaContract,/array\['audio\/\*','video\/mp4','video\/webm','video\/ogg'\]/);
assert.match(schemaContract,/select 11::integer/);
