...
function bindAudioDiagnostics(trigger){
  ...
  function text(){
    ...
    /* 상태는 늘 같은 자리에 적고, 그 밖의 값은 적은 그대로 내보낸다. 무엇을 재든
       여기를 고치지 않아도 나온다. */
    const STATE=['at','event','visibility','context','contextMode','baseLatency','outputLatency',
      'media','mediaGeneration','mediaPaused','mediaArmed','stream','actionMode','transport',
      'tempo','gain'];
    const lines=entries.map(entry=>{
      ...
      const extra=Object.keys(entry).filter(key=>STATE.indexOf(key)<0 && entry[key]!=null);
      if(extra.length) bits.push('★ '+extra.map(key=>key+'='+entry[key]).join(' · '))
      return at+'  '+bits.join(' · ');
    });
