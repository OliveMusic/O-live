...
(function(root){
  ...
  function chordName(rootPc,suffix,bassPc){
    return noteName(rootPc)+suffix+(rootPc!==bassPc?'/'+noteName(bassPc):'');
  }

  function tonesFor(pcs,rootPc,suffix){
    return pcs
      .map(pc=>({pc,interval:pcOf(pc-rootPc)}))
      .sort((a,b)=>a.interval-b.interval)
      .map(({pc,interval})=>Object.freeze({pc,name:noteName(pc),degree:degreeLabel(interval,suffix)}));
  }
  ...
  function identifyChord(notes){
    ...
    if(pcs.length===1){
      return Object.freeze({
        name:noteName(pcs[0]), root:pcs[0], bass, suffix:'',
        kind:sounding.length>1?'옥타브':'단음', interval:'',
        tones:Object.freeze(tonesFor(pcs,pcs[0],'')), alternatives:Object.freeze([]),
      });
    }
    ...
    if(!found.length){
      ...
      return Object.freeze({
        name:[bass].concat(pcs.filter(pc=>pc!==bass)).map(noteName).join(' + '),
        root:bass, bass, suffix:'',
        kind:pcs.length===2?'두 음':'이름 없음',
        interval:pcs.length===2?INTERVAL_NAMES[pcOf(other-bass)]:'',
        tones:Object.freeze(tonesFor(pcs,bass,'')), alternatives:Object.freeze([]),
      });
    }

    found.sort((a,b)=>b.score-a.score);
    const best=found[0];
    const name=chordName(best.rootPc,best.suffix,bass);
    const seen=new Set([name]);
    const alternatives=[];
    for(const candidate of found.slice(1)){
      const other=chordName(candidate.rootPc,candidate.suffix,bass);
    ...
    return Object.freeze({
      ...
      tones:Object.freeze(tonesFor(pcs,best.rootPc,best.suffix)),
