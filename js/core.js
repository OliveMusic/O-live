...
function createCtx(mode='ambient'){
  ...
  ctx.addEventListener('statechange', ()=>{
    ...
    // 재생 중 컨텍스트가 끊겼다면 가짜 '재생 중' UI를 남기지 않는다.
    // 다음 탭에서 사용자가 다시 누르면 ensureCtx()가 새 컨텍스트를 만든다.
...
function ensureCtx(mode='ambient', forceFresh=false){
  ...
  const task=previous.catch(()=>{}).then(async()=>{
    ...
    try{
      ...
      // Safari는 running이라고 남기면서 실제 장치 연결을 잃기도 한다.
      // 재생 시작 단계라면 한 번만 통째로 다시 만들어 확인한다.
      await releaseCtx();
      ctx=createCtx(mode);
      await withTimeout(resumeCtx(ctx),1400);
    }
...
/* 이미 준비된 재생 루프와 짧은 미리듣기를 위한 동기 접근자.
   resume 요청은 하나만 유지해 스케줄러가 Promise를 초당 수십 개 쌓지 않게 한다. */
function getCtx(){
...
/* ===== 소리 나는 동안 화면 켜 두기 =====
   iOS는 화면이 꺼지면 페이지 자체를 재운다. 그러면 스케줄러가 멈춰 소리도 끊긴다.
   웹에서는 이걸 뚫을 방법이 없으므로, 대신 화면이 꺼지지 않게 붙잡는다.
   연습 중에는 어차피 화면을 보고 있으니 이쪽이 맞다. */
let __wakeLock = null;
async function keepAwake(on){
  try{
    if(on){
      ...
      const w = __wakeLock; __wakeLock = null; await w.release();
    }
  }catch(e){ __wakeLock = null; }   // 지원하지 않거나 거부돼도 앱은 그대로 돌아간다
}
...
/* ===== 소리 나는 탭 표시 =====
   한 탭에서 여러 곳이 동시에 울릴 수 있으므로 출처별로 세어둔다. */
const __sounding = {};
// 박마다 호출 — 탭 발광이 템포에 맞춰 뛴다
function pulseTab(tab){
  const btn=document.querySelector('.tab-btn[data-tab="'+tab+'"]');
  if(!btn || !btn.classList.contains('sounding')) return;
  // 같은 클래스를 다시 붙여도 애니메이션은 재생되지 않는다.
  // iOS는 버튼의 폭만 읽으면 가상 요소 애니메이션을 캐시할 때가 있어,
  // ::after의 계산된 스타일을 직접 읽어 매 박마다 새로 시작하게 한다.
...
function anySounding(){
  return Object.keys(__sounding).some(k => __sounding[k].size > 0);
}

/* 소리를 내는 것들(메트로놈·리듬·잼·마이크)을 한 곳에 모아 둔다.
   화면이 꺼질 때 한꺼번에 세우기 위해서다. */
const __transports = [];
function registerTransport(t){ __transports.push(t); }
function stopAllTransports(){
  __transports.forEach(t=>{ try{ if(t.isPlaying()) t.stop(); }catch(e){} });
}
...
/* 화면을 껐다 켜면 잠금은 자동으로 풀리고, 오디오도 멈춘 채로 남는다.
   돌아왔을 때 둘 다 되살린다. */
document.addEventListener('visibilitychange', ()=>{
  if(document.visibilityState !== 'visible'){
    // 백그라운드에서는 스케줄러가 눌려 소리가 튀거나 첫박만 반복된다.
    // 상태가 running이어도 실제 장치 연결만 사라지는 WebKit 사례가 있으므로
    // 깔끔하게 멈추고 컨텍스트까지 버린다.
    stopAllTransports();
    keepAwake(false);
    releaseCtx();
  ...
  // 다음 시작 버튼의 사용자 제스처 안에서 ensureCtx()가 새 컨텍스트를 만든다.
});
