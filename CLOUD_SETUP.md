...
2. SQL Editor에서 아래 파일을 번호 순서대로 각각 한 번 실행합니다.
   ...
   마지막 파일까지 실행해야 앱의 클라우드 상태가 `DB-005` 없이 정상으로 표시됩니다.
...
배포된 함수는 로그인 사용자의 JWT를 확인한 뒤 그 사용자 계정만 삭제합니다.
...
- 클라우드 데이터 삭제: 기록과 설정을 지우고 계정은 유지합니다.
- 계정 삭제: Edge Function이 Supabase 계정과 연결된 모든 데이터를 함께 삭제합니다.
